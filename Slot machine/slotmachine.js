...
const getbet = (numberdepositamount, numberoflines) => {
  ...
  while (true) {
    ...
    if (
      isNaN(numberbet) ||
      numberbet <= 0 ||
      numberbet > numberdepositamount / numberoflines
    ) {
...
const spin = () => {
  ...
  for (const [symbol, count] of Object.entries(SYMBOLS_COUNT)) {
    //using Object.entries method and
  ...
  for (let i = 0; i < COLS; i++) {
    //pushing arr into arr equal to no of cols, to make a matrix(wheel)
    reels.push([]);
...
printrows(rows);
const winnings = getWinnings(rows, fnbet, fnnumberoflines);
